...
function FlyToVillage({ village }: { village: Village | null }) {
  const map = useMap();
  const previousVillageRef = useRef<Village | null>(null);

  useEffect(() => {
    if (village && village.id !== previousVillageRef.current?.id) {
      map.flyTo([village.lat, village.lng], 13, { duration: 0.8 });
      previousVillageRef.current = village;
    }
  }, [map, village]);
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-600">
                Sangat Prioritas (&gt;0.7)
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-warning" />
              <span className="text-[11px] text-slate-600">
                Prioritas Sedang (0.4-0.7)
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-success" />
              <span className="text-[11px] text-slate-600">
                Prioritas Rendah (&lt;0.4)
              </span>
            </div>
          </div>
        </div>
