...
export default function MasterDataModal({
  ...
  const saveVillage = () => {
    ...
    setVillages((previous) => {
      const existingIndex = previous.findIndex(
        (village) => String(village.id) === String(editingVillage.id),
      );

      if (existingIndex !== -1) {
        const next = [...previous];
        next[existingIndex] = editingVillage;
        return next;
      }

      return [editingVillage, ...previous];
    });
  ...
  const handleEditWeight = (weight: WeightFormat) => {
    setAhpState({ isOpen: true, format: weight });
  };
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {weights.map((weight, index) => (
                        <tr key={weight.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3 text-slate-500">{index + 1}</td>
                          <td className="px-4 py-3 font-medium text-slate-700 max-w-[120px] truncate" title={weight.name}>
                            {weight.name}
                          </td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.komitmen.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.remaja.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.stunting.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.prevalensi.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.kemiskinan.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.jarak.toFixed(3)}</td>
                          <td className="px-2 py-3 text-center text-slate-600 font-mono text-[10px]">{weight.weights.tenagaKerja.toFixed(3)}</td>
      ...
      {isVillageModalOpen && editingVillage && (
        <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm z-[10010] flex items-center justify-center p-4 rounded-2xl">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 animate-in fade-in zoom-in-95 max-h-[90vh] flex flex-col">
            <h3 className="text-sm font-bold text-slate-800 mb-4 flex-shrink-0">
              {editingVillage.name ? "Edit Desa" : "Tambah Desa"}
            </h3>
            <div className="space-y-4 overflow-y-auto flex-1 pr-1">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs border border-slate-300 rounded-md px-3 py-2 bg-slate-50 focus:bg-white focus:border-primary outline-none transition-colors"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      max="5"
                      value={editingVillage.komitmen ?? 3}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          komitmen: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-slate-700 mb-1">
                    ...
                    <input
                      type="number"
                      value={editingVillage.remaja ?? 15}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          remaja: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingVillage.stunting ?? 10}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          stunting: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingVillage.prevalensi ?? 20}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          prevalensi: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingVillage.kemiskinan ?? 15}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          kemiskinan: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingVillage.jarak ?? 2}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          jarak: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      type="number"
                      value={editingVillage.tenagaKerja ?? 100}
                      onChange={(event) =>
                        setEditingVillage({
                          ...editingVillage,
                          tenagaKerja: Number(event.target.value),
                        })
                      }
                      className="w-full text-xs font-medium border border-slate-300 rounded-md px-2 py-1.5 bg-slate-50 focus:bg-white focus:border-primary outline-none"
                    />
      ...
      {ahpState.isOpen && (
        <AHPWeightCalculator
          ...
          onSave={(name, weightsResult, preferences, idToUpdate) => {
